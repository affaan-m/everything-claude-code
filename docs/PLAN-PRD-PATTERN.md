...
This pattern replaces the former `prp-*` command set (`prp-prd`, `prp-plan`, `prp-implement`, `prp-commit`, `prp-pr`) ported from PRPs-agentic-eng. That set was collapsed into ECC-native commands because:

- `prp-implement` overlapped entirely with `/tdd`.
- `prp-commit` was a thin natural-language wrapper over `git commit`.
- `prp-plan`'s pattern-extraction was valuable and merged into `/plan`.
- `prp-pr` filled a genuine gap (no native PR creation) and was renamed to `/pr`.
- `prp-prd` was rewritten as `/plan-prd`, scoped to requirements only (no embedded implementation planning), and made optional.
