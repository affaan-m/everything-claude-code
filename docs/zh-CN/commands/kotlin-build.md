...
```bash
$ ./gradlew build
e: src/main/kotlin/com/example/service/UserService.kt:25:15 未解析的引用：UserRepository
e: src/main/kotlin/com/example/routes/UserRoutes.kt:42:9 类型不匹配：推断类型为 String，但期望的是 Int
e: src/main/kotlin/com/example/routes/UserRoutes.kt:58:5 'when' 表达式必须穷尽所有可能
```
...
错误：类型不匹配：推断类型为 String，但期望的是 Int
...
val count = call.parameters["count"]?.toIntOrNull()
    ?: return@get call.respond(HttpStatusCode.BadRequest, "无效的计数")
```
