...
如果未找到 PR，则停止并报错。存储 PR 元数据以供后续阶段使用。
...
2. **PRP 产物** — 检查 `.claude/PRPs/reports/` 和 `.claude/PRPs/plans/` 以获取与此 PR 相关的实现上下文
3. **PR 意图** — 解析 PR 描述以获取目标、关联问题、测试计划
4. **更改的文件** — 列出所有修改的文件并按类型分类（源代码、测试、配置、文档）

### 阶段 3 — 审查

**完整**阅读每个更改的文件（不仅仅是差异块 — 你需要周围的上下文）。
...
| **安全性** | 注入、认证漏洞、密钥泄露、SSRF、路径遍历、XSS |
| **性能** | N+1 查询、缺少索引、无限循环、内存泄漏、大型负载 |
...
* 显式的 `--approve` 或 `--request-changes` 标志 → 覆盖决策（但仍报告所有发现）
...
* **大型 PR（超过 50 个文件）**：警告审查范围。首先关注源代码更改，然后是测试，最后是配置/文档。
