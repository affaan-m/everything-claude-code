...
你可以使用常规的 React 代码在 Remotion 中创建柱状图——支持 HTML 和 SVG，以及 D3.js。

## 不由 `useCurrentFrame()` 驱动的动画

禁用所有第三方库的动画。
它们会在渲染过程中导致闪烁。
相反，所有动画都应通过 `useCurrentFrame()` 驱动。

## 柱状图动画

参见[柱状图示例](../../../../../skills/remotion-video-creation/rules/assets/charts/bar-chart.tsx)了解基本实现。

### 交错柱状图

你可以像这样为柱状图的高度添加动画并实现交错效果：
