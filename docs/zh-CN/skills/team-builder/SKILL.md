...
代理通过两种方式发现，并按代理名称合并去重：

1. **`claude agents` 命令**（主要方式）—— 运行 `claude agents` 获取 CLI 已知的所有代理，包括用户代理、插件代理（例如 `everything-claude-code:architect`）和内置代理。此方式无需任何路径配置即可自动覆盖 ECC 市场安装的代理。
2. **文件通配符**（备用方式，用于读取代理内容）—— 代理 Markdown 文件从以下位置读取：
   * `./agents/**/*.md` + `./agents/*.md` —— 项目本地代理
   * `~/.claude/agents/**/*.md` + `~/.claude/agents/*.md` —— 全局用户代理

名称冲突时，较早的来源优先：用户代理 > 插件代理 > 内置代理。如果用户指定了自定义路径，则使用该路径替代。
...
* **插件代理** 以 `plugin-name:` 为前缀（例如 `everything-claude-code:security-reviewer`）。使用 `:` 之后的部分作为代理名称，插件名称作为域。
* **用户代理** 没有前缀。从 `~/.claude/agents/` 或 `./agents/` 读取对应的 Markdown 文件，提取名称和描述。
...
如果在运行 `claude agents` 并探测文件位置后未找到任何代理，则通知用户："未找到代理。请运行 `claude agents` 验证您的设置。" 然后停止。
