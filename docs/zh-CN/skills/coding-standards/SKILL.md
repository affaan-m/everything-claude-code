...
description: 跨项目基线编码规范，涵盖命名、可读性、不可变性和代码质量审查。使用详细的前端或后端技能来处理框架特定模式。
origin: ECC
---
...
此技能是共享基础，而非详细的框架操作手册。

* 使用 `frontend-patterns` 处理 React、状态、表单、渲染和 UI 架构。
* 使用 `backend-patterns` 或 `api-design` 处理仓库/服务层、端点设计、验证和服务器特定问题。
* 当你需要最短的可复用规则层而非完整的技能指南时，使用 `rules/common/coding-style.md`。
