...
> 本文在 [common/patterns.md](../common/patterns.md) 基础上扩展了 Dart、Flutter 及常见生态系统的特定内容。
...
优先使用构造函数注入。在组合根处使用 `get_it` 或 Riverpod 提供者：
...
├── data/                # 实现领域层接口
│   ├── datasources/
│   ├── models/          # 包含 fromJson/toJson 的数据传输对象
│   └── repositories/
...
参见技能：`flutter-dart-code-review` 获取完整审查清单。
参见技能：`compose-multiplatform-patterns` 获取 Kotlin Multiplatform/Flutter 互操作模式。
