...
技能是主要的工作流界面。它们就像作用域限定的工作流包：当你需要特定执行模式时，可复用的提示词、结构、支持文件和代码映射。

在使用 Opus 4.5 进行长时间编码后，想要清理死代码和零散的 .md 文件？运行 `/refactor-clean`。需要测试？`/tdd`、`/e2e`、`/test-coverage`。这些斜杠命令很方便，但真正持久化的单元是底层的技能。技能还可以包含代码映射——这是让 Claude 快速浏览代码库而无需消耗上下文进行探索的方式。

![显示链式命令的终端](../../assets/images/shortform/02-chaining-commands.jpeg)
*将命令链接在一起*

ECC 仍然提供 `commands/` 层，但最好将其视为迁移期间的遗留斜杠命令兼容方案。持久化的逻辑应该存在于技能中。

* **技能**：`~/.claude/skills/` - 规范的工作流定义
* **命令**：`~/.claude/commands/` - 当你仍需要时使用的遗留斜杠命令垫片
