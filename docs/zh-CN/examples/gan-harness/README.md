...
|---------|---------|---------|------|---------|
| 全栈Web应用 | playwright | 10-15 | 7.0 | $100-200 |
| 落地页 | screenshot | 5-8 | 7.5 | $30-60 |
| REST API | code-only | 5-8 | 7.0 | $30-60 |
| CLI工具 | code-only | 3-5 | 6.5 | $15-30 |
...
2. **`gan-harness/feedback/`** — 所有评估反馈（有助于理解质量演变）
3. **`gan-harness/spec.md`** — 完整规格说明（如需手动继续开发）
4. **分数进展** — 应显示稳定提升。平台期表明模型已达到能力上限。

## 使用技巧

1. **从清晰的需求说明开始** — "用Y和Z构建X"优于"做点酷的东西"
2. **迭代次数不要低于5次** — 前2-3次迭代通常低于阈值
3. **UI项目使用`playwright`模式** — 仅截图会遗漏交互错误
4. **审查反馈文件** — 即使最终分数通过，反馈中仍包含宝贵见解
5. **迭代优化规格说明** — 若结果不理想，改进`spec.md`并使用`--skip-planner`重新运行
