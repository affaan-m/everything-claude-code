...
- **Публичный контур синхронизирован с текущим репозиторием** — метаданные, счётчики каталога, манифесты плагинов и документация для установки теперь соответствуют реальному OSS-набору: 48 агентов, 182 навыка и 68 устаревших совместимых заглушек команд.
...
- идентификатор Claude marketplace/plugin: `everything-claude-code@everything-claude-code`
- npm-пакет: `ecc-universal`

Это сделано намеренно. Установки Anthropic marketplace/plugin ключуются каноническим идентификатором плагина, поэтому ECC стандартизировался на `everything-claude-code@everything-claude-code`, чтобы имя листинга, `/plugin install`, `/plugin list` и документация репозитория указывали на один публичный путь установки. Старые публикации могут всё ещё показывать прежний короткий никнейм; эта короткая форма устарела. Отдельно npm-пакет остался `ecc-universal`, поэтому npm-установки и marketplace-установки намеренно используют разные имена.
...
/everything-claude-code:plan "Добавить аутентификацию пользователей"
...
**Готово.** Теперь у вас есть доступ к 48 агентам, 182 навыкам и 68 устаревшим совместимым заглушкам команд.
...
|
|-- agents/           # 36 специализированных субагентов для делегирования
...
|-----------|---------------|-------|
| Спланировать новую функцию | `/everything-claude-code:plan "Добавить auth"` | planner |
| Спроектировать архитектуру системы | `/everything-claude-code:plan` + агент architect | architect |
...
```
/everything-claude-code:plan "Добавить OAuth-аутентификацию пользователей"
...
| Agents | 48 | `.cursor/agents/ecc-*.md` при установке; с префиксом, чтобы избежать конфликтов с user или marketplace agents |
...
|-------------|-------------|----------|--------|
| Agents | PASS: 48 agents | PASS: 12 agents | **Claude Code впереди** |
| Commands | PASS: 68 commands | PASS: 31 commands | **Claude Code впереди** |
| Skills | PASS: 182 skills | PASS: 37 skills | **Claude Code впереди** |
