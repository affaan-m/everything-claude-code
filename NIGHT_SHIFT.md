# 🌙 Night Shiftモード - 自律的タスク実行ガイド

Night Shiftモードは、夜間や無人の期間中にClaude Codeがタスクリストを自律的に処理できるようにします。
...
1. 計画
   → planner エージェントが実装計画を作成
   → 計画をCURRENT_TASK.mdに保存

2. 実装
   → tdd-guide エージェントが計画に従う
   → テストを先に書く
   → 機能を実装
   → すべてのテストがパス ✓

3. レビュー
   → code-reviewer エージェントが品質をチェック
   → 改善を提案
   → 修正を適用

4. セキュリティ
   → security-reviewer エージェントが認証実装をチェック
   → パスワードハッシュ、セッション管理を検証
   → セキュリティ ✓
