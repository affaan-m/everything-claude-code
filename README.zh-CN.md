...
# 或仅为指定编程语言安装
./install.sh typescript    # 也可安装 python、golang、swift、php、harmonyos
# ./install.sh typescript python golang swift php harmonyos
...
# 或仅为指定编程语言安装
.\install.ps1 typescript   # 也可安装 python、golang、swift、php、harmonyos
# .\install.ps1 typescript python golang swift php harmonyos
...
|   |-- harmonyos-app-resolver.md # HarmonyOS/ArkTS 应用开发（新增）
|
|-- skills/           # 工作流定义与领域知识库
...
|   |-- swift/               # Swift 专属规范
|   |-- php/                 # PHP 专属规范（新增）
|   |-- harmonyos/           # HarmonyOS / ArkTS 专属规范（新增）
|
|-- hooks/            # 基于触发器的自动化逻辑
...
|   |-- harmonyos-app-CLAUDE.md   # HarmonyOS 应用（ArkTS + ArkUI + V2 状态管理）（新增）
|
...
> cp -r everything-claude-code/rules/harmonyos ~/.claude/rules/   # HarmonyOS/ArkTS 开发
>
> # 方案 B：项目级规则（仅对当前项目生效）
> mkdir -p .claude/rules
> cp -r everything-claude-code/rules/common .claude/rules/
...
~/.claude/rules/
  ...
  golang/          # Go 特定模式和工具
  perl/            # Perl 特定模式和工具
  harmonyos/       # HarmonyOS / ArkTS 特定模式和约束
```
...
- 特定语言技能（Rust、C#、Kotlin、Java）—— Go、Python、Perl、Swift、TypeScript 和 HarmonyOS/ArkTS 已内置
