...
function createTestDir() {
  const testDir = path.join(os.tmpdir(), `hook-integration-test-${Date.now()}`);
...
async function runTests() {
  ...
  if (await asyncTest('hooks handle null/undefined values in input', async () => {
    const result = await runHookWithInput(
      path.join(scriptsDir, 'session-start.js'),
      { tool_input: null, tool_output: undefined }
    );

    assert.strictEqual(result.code, 0, 'Should handle null/undefined gracefully');
  })) passed++; else failed++;
