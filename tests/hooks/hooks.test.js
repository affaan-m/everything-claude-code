...
function runScript(scriptPath, input = "", env = {}, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn("node", [scriptPath], spawnOpts);
...
async function runTests() {
  ...
  if (
    await asyncTest("exits 0 even with isolated empty HOME", async () => {
      ...
      fs.mkdirSync(path.join(isoHome, ".claude", "sessions"), {
        recursive: true,
      });
      fs.mkdirSync(path.join(isoHome, ".claude", "skills", "learned"), {
        recursive: true,
      });
      try {
  ...
  if (
    await asyncTest("skips template session content", async () => {
      ...
      const sessionFile = path.join(
        sessionsDir,
        "2026-02-11-abcd1234-session.tmp",
      );
      fs.writeFileSync(
        sessionFile,
  ...
  if (
    await asyncTest("injects real session content", async () => {
      ...
      const sessionFile = path.join(
        sessionsDir,
        "2026-02-11-efgh5678-session.tmp",
      );
      fs.writeFileSync(
        sessionFile,
  ...
  if (
    await asyncTest("passes through stdin data to stdout", async () => {
      ...
      assert.strictEqual(result.code, 0);
      assert.ok(
        result.stdout.includes("tool_name"),
        "Should pass through stdin data",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest("handles invalid JSON stdin gracefully", async () => {
      ...
      assert.ok(
        ...
        "Should pass through invalid data",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest("creates or updates session file", async () => {
      ...
      const sessionFile = path.join(
        sessionsDir,
        `${today}-${expectedId}-session.tmp`,
      );

      assert.ok(
        fs.existsSync(sessionFile),
        `Session file should exist: ${sessionFile}`,
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest("includes session ID in filename", async () => {
      ...
      const sessionFile = path.join(
        sessionsDir,
        `${today}-${expectedShortId}-session.tmp`,
      );
  ...
  if (
    await asyncTest("outputs PreCompact message", async () => {
      const result = await runScript(path.join(scriptsDir, "pre-compact.js"));
      assert.ok(
        result.stderr.includes("[PreCompact]"),
        "Should output PreCompact message",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const isoHome = path.join(
          os.tmpdir(),
          `ecc-compact-annotate-${Date.now()}`,
        );
        const sessionsDir = path.join(isoHome, ".claude", "sessions");
        fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest("handles corrupted counter file", async () => {
      ...
      const newCount = parseInt(
        fs.readFileSync(counterFile, "utf8").trim(),
        10,
      );
      assert.strictEqual(
        newCount,
        1,
        "Should reset counter to 1 on corrupt data",
      );
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes("15 messages"),
          "Should count user messages with whitespace in JSON, got: " +
            result.stderr.trim(),
        );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should handle null content elements without crash",
        );
      },
    )
  )
  ...
  if (
    await asyncTest("warns about console.log in JS files", async () => {
      ...
      const result = await runScript(
        path.join(scriptsDir, "post-edit-console-warn.js"),
        stdinJson,
      );
  ...
  if (
    await asyncTest("does not warn for non-JS files", async () => {
      ...
      assert.ok(
        !result.stderr.includes("console.log"),
        "Should not warn for non-JS files",
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest("does not warn for clean JS files", async () => {
      ...
      assert.ok(
        ...
        "Should not warn for clean files",
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest("limits console.log output to 5 matches", async () => {
      ...
      assert.ok(
        debugLines.length <= 5,
        `Should show at most 5 matches, got ${debugLines.length}`,
      );
      // Should include debug 1 but not debug 8 (sliced)
      assert.ok(
        result.stderr.includes("debug 1"),
        "Should include first match",
      );
      assert.ok(
        !result.stderr.includes("debug 8"),
        "Should not include 8th match",
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest("passes through original data on stdout", async () => {
      ...
      assert.ok(
        result.stdout.includes("tool_input"),
        "Should pass through stdin data",
      );
    })
  )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 even when prettier fails",
        );
        assert.ok(
          result.stdout.includes("tool_input"),
  ...
  if (
    await asyncTest("handles TS file with no tsconfig gracefully", async () => {
      ...
      assert.strictEqual(
        result.code,
        0,
        "Should exit 0 when no tsconfig found",
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest("stops tsconfig walk at max depth (20)", async () => {
      ...
      assert.ok(
        elapsed < 5000,
        `Should complete quickly at depth limit, took ${elapsed}ms`,
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stdout.includes("tool_input"),
          "Should pass through stdin data on stdout",
        );
        cleanupTestDir(testDir);
      },
    )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should handle array content without crash",
        );
        cleanupTestDir(testDir);
      },
    )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes("unparseable") ||
  ...
  if (
    await asyncTest("handles empty transcript (no user messages)", async () => {
      ...
      assert.strictEqual(
        result.code,
        0,
        "Should handle transcript with no user messages",
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest("truncates long user messages to 200 chars", async () => {
      ...
      assert.strictEqual(
        result.code,
        0,
        "Should handle and truncate long messages",
      );
      cleanupTestDir(testDir);
    })
  )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should handle backticks without crash",
        );
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            const content = fs.readFileSync(
              path.join(claudeDir, files[0]),
              "utf8",
            );
            // Backticks should be escaped in the output
            assert.ok(
              content.includes("\\`"),
              "Should escape backticks in session file",
            );
            assert.ok(
              !content.includes("`handleAuth`"),
              "Raw backticks should be escaped",
            );
          }
        }
        cleanupTestDir(testDir);
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              ...
              "Should list edited file",
            );
            assert.ok(
              content.includes("/src/new-file.ts"),
              "Should list written file",
            );
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              ...
              "Should NOT have Tools Used when empty",
            );
            assert.ok(
    ...
    await asyncTest("slices user messages to last 10", async () => {
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            !content.includes("UserMsg_1\n"),
            "Should not include first message (sliced)",
          );
          assert.ok(
            !content.includes("UserMsg_5\n"),
            "Should not include 5th message (sliced)",
          );
          // Should contain messages 6-15
          assert.ok(
            content.includes("UserMsg_6"),
            "Should include 6th message",
          );
          assert.ok(
            content.includes("UserMsg_15"),
            "Should include last message",
          );
          assert.ok(
            content.includes("Total user messages: 15"),
  ...
  if (
    await asyncTest("slices tools to first 20", async () => {
      ...
      for (let i = 1; i <= 25; i++) {
        lines.push(
          `{"type":"tool_use","tool_name":"Tool${i}","tool_input":{}}`,
        );
      }
      fs.writeFileSync(transcriptPath, lines.join("\n"));
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            !content.includes("Tool21"),
            "Should not include Tool21 (sliced to 20)",
          );
          assert.ok(
            !content.includes("Tool25"),
            "Should not include Tool25 (sliced to 20)",
          );
        }
      }
      cleanupTestDir(testDir);
    ...
    await asyncTest("slices files modified to first 30", async () => {
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            content.includes("/src/file30.ts"),
            "Should include file30",
          );
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              ...
              "Should extract Edit tool from content blocks",
            );
            assert.ok(
              content.includes("/src/app.ts"),
              "Should extract file path from Edit block",
            );
            assert.ok(
              content.includes("/src/new.ts"),
              "Should extract file path from Write block",
            );
          }
        }
        cleanupTestDir(testDir);
      ...
      const checkHooks = (hookArray) => {
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            if (
              hook.type === "command" &&
              hook.command.includes("scripts/hooks/")
            ) {
              // Check for the literal string "${CLAUDE_PLUGIN_ROOT}" in the command
              const hasPluginRoot = hook.command.includes(
                "${CLAUDE_PLUGIN_ROOT}",
              );
              assert.ok(
                hasPluginRoot,
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stderr.includes("12 messages"),
          "Should report message count",
        );
        assert.ok(
          result.stderr.includes("evaluate"),
          "Should signal evaluation",
        );
        cleanupTestDir(testDir);
      },
    )
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            newCount,
            1,
            "Should reset to 1 on overflow value",
          );
        } finally {
          try {
            fs.unlinkSync(counterFile);
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            newCount,
            1,
            "Should reset to 1 on negative value",
          );
        } finally {
          try {
            fs.unlinkSync(counterFile);
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.stdout,
          stdinData,
          "Should not append extra newline to output",
        );
      },
    )
  )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          formatSource.includes(".cmd"),
          "Should handle .cmd suffix for Windows cross-platform safety",
        );
      },
    )
  )
  ...
  if (
    await asyncTest("matches .tsx extension for formatting", async () => {
      const stdinJson = JSON.stringify({
        tool_input: { file_path: "/nonexistent/component.tsx" },
      });
      const result = await runScript(
        path.join(scriptsDir, "post-edit-format.js"),
        stdinJson,
      ...
      assert.ok(
        result.stdout.includes("component.tsx"),
        "Should pass through data for .tsx files",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest("matches .jsx extension for formatting", async () => {
      const stdinJson = JSON.stringify({
        tool_input: { file_path: "/nonexistent/component.jsx" },
      });
      const result = await runScript(
        path.join(scriptsDir, "post-edit-format.js"),
        stdinJson,
      ...
      assert.strictEqual(result.code, 0);
      assert.ok(
        result.stdout.includes("component.jsx"),
        "Should pass through data for .jsx files",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        // Strip comments to avoid matching "shell: true" in comment text
        const codeOnly = typecheckSource
          .replace(/\/\/.*$/gm, "")
          .replace(/\/\*[\s\S]*?\*\//g, "");
        assert.ok(
          !codeOnly.includes("shell:"),
  ...
  const realUtilsPath = path.resolve(
    __dirname,
    "..",
    "..",
    "scripts",
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes("2 messages") &&
            result.stderr.includes("evaluate"),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes("too short"),
          "Should fall back to default 10 when null",
        );
        cleanupTestDir(testDir);
      },
    )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          fs.existsSync(customLearnedDir),
          "Should create custom learned skills directory",
        );
        cleanupTestDir(testDir);
      },
    )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          updated.includes("**Last Updated:**"),
          "Should still have Last Updated field",
        );
        assert.ok(
          result.stderr.includes("Updated session file"),
          "Should log update",
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          updated.includes("Fix auth bug"),
          "Should include user message in summary",
        );
        assert.ok(
          updated.includes("/src/auth.ts"),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          !updated.includes("New task"),
          "Should not replace non-template content",
        );
      },
    )
  )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            sessionContent.includes("Compaction occurred"),
            "Should annotate session file",
          );
          assert.strictEqual(
            otherContent,
            "some other data\n",
  ...
  if (
    await asyncTest("handles no active session files gracefully", async () => {
      const isoHome = path.join(
        os.tmpdir(),
        `ecc-compact-nosession-${Date.now()}`,
      );
      const sessionsDir = path.join(isoHome, ".claude", "sessions");
      fs.mkdirSync(sessionsDir, { recursive: true });
      ...
      try {
        const result = await runScript(
          path.join(scriptsDir, "pre-compact.js"),
          "",
          {
            HOME: isoHome,
            USERPROFILE: isoHome,
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 with no session files",
        );
        assert.ok(
          result.stderr.includes("[PreCompact]"),
        ...
        assert.ok(
          ...
          "Should create compaction log even with no sessions",
        );
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
      ...
      async () => {
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              content.includes("/src/new.ts"),
              "Should extract Write file from nested block",
            );
            assert.ok(
              content.includes("Edit"),
              "Should list Edit in tools used",
            );
          }
        }
        cleanupTestDir(testDir);
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            newCount,
            1,
            "Should reset to 1 on corrupted file content",
          );
        } finally {
          try {
            fs.unlinkSync(counterFile);
    ...
    await asyncTest("handles counter at exact 1000000 boundary", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          newCount,
          1000001,
          "Should increment from exactly 1000000",
        );
      } finally {
        try {
          fs.unlinkSync(counterFile);
  ...
  if (
    await asyncTest("passes through malformed JSON unchanged", async () => {
      ...
      assert.ok(
        result.stdout.includes(malformedJson),
        "Should pass through malformed JSON",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stdout.includes("file.py"),
          "Should pass through for .py files",
        );
      },
    )
  )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const stdinJson = JSON.stringify({
          tool_input: { file_path: "/nonexistent/deep/file.ts" },
        });
        const result = await runScript(
          path.join(scriptsDir, "post-edit-typecheck.js"),
          stdinJson,
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stdout.includes("file.ts"),
          "Should pass through for non-existent .ts file",
        );
      },
    )
  )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.stdout,
          malformed,
          "Should pass through malformed JSON exactly",
        );
      },
    )
  )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.stdout,
          stdinJson,
          "Should pass through with missing file_path",
        );
      },
    )
  )
  ...
  if (
    await asyncTest("source has expected exclusion patterns", async () => {
      ...
      assert.ok(
        source.includes("EXCLUDED_PATTERNS"),
        "Should have exclusion patterns array",
      );
      assert.ok(
        /\.test\\\./.test(source),
        "Should have test file exclusion pattern",
      );
      assert.ok(
        /\.spec\\\./.test(source),
        "Should have spec file exclusion pattern",
      );
      assert.ok(
        source.includes("scripts"),
        "Should exclude scripts/ directory",
      );
      assert.ok(
        source.includes("__tests__"),
        "Should exclude __tests__/ directory",
      );
      assert.ok(
        source.includes("__mocks__"),
        "Should exclude __mocks__/ directory",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stdout.includes(stdinData),
          "Should pass through data",
        );
        cleanupTestDir(testDir);
      },
  ...
  if (
    await asyncTest("source uses process.cwd() as cwd for npx", async () => {
      const formatSource = fs.readFileSync(
        path.join(scriptsDir, "post-edit-format.js"),
        "utf8",
      );
      assert.ok(
        formatSource.includes("cwd"),
        "Should set cwd option for execFileSync",
      );
      assert.ok(
        formatSource.includes("process.cwd()"),
        "cwd should use process.cwd()",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      "source calls process.exit(0) after writing output",
      async () => {
        const cwSource = fs.readFileSync(
          path.join(scriptsDir, "post-edit-console-warn.js"),
          "utf8",
        );
        assert.ok(
          cwSource.includes("process.exit(0)"),
  ...
  if (
    await asyncTest("does NOT match .mts or .mjs extensions", async () => {
      const stdinMts = JSON.stringify({
        tool_input: { file_path: "/some/file.mts" },
      });
      const result = await runScript(
        path.join(scriptsDir, "post-edit-console-warn.js"),
        stdinMts,
      ...
      assert.strictEqual(
        result.stdout,
        stdinMts,
        "Should pass through .mts without scanning",
      );
      assert.ok(
        !result.stderr.includes("console.log"),
  ...
  if (
    await asyncTest("does NOT match uppercase .TS extension", async () => {
      ...
      assert.strictEqual(
        result.stdout,
        stdinTS,
        "Should pass through .TS without scanning",
      );
      assert.ok(
        !result.stderr.includes("console.log"),
        "Should NOT scan .TS (uppercase) files",
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      "source calls process.exit(0) after writing output",
      async () => {
        const clSource = fs.readFileSync(
          path.join(scriptsDir, "check-console-log.js"),
          "utf8",
        );
      ...
      async () => {
        ...
        for (const substr of expectedSubstrings) {
          assert.ok(
            ...
            `Should include pattern containing "${substr}"`,
          );
        }
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const runAllSource = fs.readFileSync(
          path.join(__dirname, "..", "run-all.js"),
          "utf8",
        );
        assert.ok(
          runAllSource.includes("spawnSync"),
          "Should use spawnSync instead of execSync",
        );
        assert.ok(
          !runAllSource.includes("execSync"),
          "Should not use execSync",
        );
        // Verify it shows stderr
        assert.ok(
          runAllSource.includes("stderr"),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should not crash on shell metacharacters",
        );
        // execFileSync prevents shell injection — just verify no crash
        assert.ok(
          result.stdout.includes("tool_input"),
          "Should pass through data safely",
        );
        cleanupTestDir(testDir);
      },
    )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          !result.stderr.includes("WARNING"),
          "Should NOT warn for console.error/warn only",
        );
        cleanupTestDir(testDir);
      },
    )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should handle whitespace-only transcript",
        );
        cleanupTestDir(testDir);
      },
    )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const isoHome = path.join(
          os.tmpdir(),
          `ecc-compact-multi-${Date.now()}`,
        );
        const sessionsDir = path.join(isoHome, ".claude", "sessions");
        fs.mkdirSync(sessionsDir, { recursive: true });

        // Create two session files with different mtimes
        const olderSession = path.join(
          sessionsDir,
          "2026-01-01-older-session.tmp",
        );
        const newerSession = path.join(
          sessionsDir,
          "2026-02-11-newer-session.tmp",
        );
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 with empty session file",
          );
  ...
  if (
    await asyncTest(".mts extension does not trigger typecheck", async () => {
      const stdinJson = JSON.stringify({
        tool_input: { file_path: "/project/utils.mts" },
      });
      const result = await runScript(
        path.join(scriptsDir, "post-edit-typecheck.js"),
        stdinJson,
      ...
      assert.strictEqual(
        result.stdout,
        stdinJson,
        "Should pass through .mts unchanged",
      );
    })
  )
    passed++;
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            !content.includes("### Files Modified"),
            "Should omit Files Modified when empty",
          );
          assert.ok(
            !content.includes("### Tools Used"),
            "Should omit Tools Used when empty",
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
          cleanupTestDir(testDir);
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(result.code, 0);
          assert.ok(
            result.stderr.includes("alias"),
            "Should mention aliases in stderr",
          );
          assert.ok(
            result.stderr.includes("my-feature") ||
              result.stderr.includes("bug-fix"),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          results.forEach((r, i) =>
            assert.strictEqual(r.code, 0, `Run ${i} should exit 0`),
          );

          const logFile = path.join(sessionsDir, "compaction-log.txt");
          assert.ok(fs.existsSync(logFile), "Compaction log should exist");
          const content = fs.readFileSync(logFile, "utf8");
          const entries = (content.match(/Context compaction triggered/g) || [])
            .length;
          assert.strictEqual(
            entries,
            3,
            `Should have 3 log entries, got ${entries}`,
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
        }
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 even when sessions dir is blocked",
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
        }
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          lineReports,
          5,
          `Should report max 5 matches, got ${lineReports}`,
        );
        cleanupTestDir(testDir);
      },
    )
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 for non-existent file",
        );
        assert.strictEqual(
          result.stdout,
          stdinJson,
          "Should pass through stdin data unchanged",
        );
      },
    )
  )
  ...
  if (
    await asyncTest("excludes session files older than 7 days", async () => {
      ...
      const recentFile = path.join(
        sessionsDir,
        "2026-02-06-recent69-session.tmp",
      );
      fs.writeFileSync(recentFile, "# Recent Session\n\nRECENT CONTENT HERE");
      const sixPointNineDaysAgo = new Date(
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        const olderSession = path.join(
          sessionsDir,
          "2026-02-11-olderabc-session.tmp",
        );
        fs.writeFileSync(
          olderSession,
        ...
        const newerSession = path.join(
          sessionsDir,
          "2026-02-12-newerdef-session.tmp",
        );
        fs.writeFileSync(
          newerSession,
    ...
    await asyncTest(
      ...
      async () => {
        ...
        const oversizedPayload =
          '{"transcript_path":"' + "x".repeat(1048600) + '"}';
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 even with oversized stdin",
          );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 after walking up to find tsconfig",
        );
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        fs.writeFileSync(
          sessionFile,
          "# Sensitive session content that should NOT appear",
        );
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 even with unreadable session file",
          );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          ...
          "Should still pass through truncated data",
        );
      },
    )
  )
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 even when append fails",
          );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 even when replaceInFile fails",
        );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 for JSON without tool_input",
        );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes("Deploy the production build") ||
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(
            result.code,
            0,
            "Should exit 0 for missing transcript",
          );
          assert.ok(
            result.stderr.includes("Transcript not found") ||
          ...
          assert.ok(
            files.length > 0,
            "Should still create session file even without transcript",
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
        }
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes("Edit"),
            "Should list Edit via entry.name fallback",
          );
          assert.ok(
            content.includes("Read"),
            "Should list Read via entry.name fallback",
          );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes("Refactor the auth module") ||
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes("Edit"),
            "Should collect Edit tool via tool_name OR fallback",
          );
...
`;
    ...
    await asyncTest(
      ...
      async () => {
        ...
        assert.strictEqual(
          result.code,
          0,
          "Should exit 0 even when tsc finds errors",
        );
        const parsed = JSON.parse(result.stdout);
        assert.strictEqual(
          parsed.tool_input.file_path,
          ...
          child.on("close", (code) => {
            ...
            try {
              assert.strictEqual(
                code,
                0,
                "Should exit 0 via timeout resolution",
              );
      ...
      async () => {
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              content.includes("### Tools Used"),
              "Should include Tools Used section",
            );
            assert.ok(content.includes("Read"), "Should list Read tool");
  ...
  // ── Round 95: post-edit-format.js Biome support (cwd-based config detection) ──
  console.log("\nRound 95: post-edit-format.js (Biome support):");

  if (
    await asyncTest("uses biome when biome.json exists in cwd", async () => {
      const testDir = createTestDir();
      ...
      assert.strictEqual(
        result.code,
        0,
        "Should exit 0 with biome.json in cwd",
      );
      assert.ok(
        result.stdout.includes("tool_input"),
        "Should pass through stdin data",
  ...
  if (
    await asyncTest("uses biome when biome.jsonc exists in cwd", async () => {
      const testDir = createTestDir();
      ...
      assert.strictEqual(
        result.code,
        0,
        "Should exit 0 with biome.jsonc in cwd",
      );
      assert.ok(
        result.stdout.includes("tool_input"),
        "Should pass through stdin data",
  ...
  if (
    await asyncTest(
      "source uses process.cwd() for biome detection, not walk-up",
      async () => {
        const formatSource = fs.readFileSync(
          path.join(scriptsDir, "post-edit-format.js"),
          "utf8",
        ...
        assert.ok(
          formatSource.includes("process.cwd()"),
          "Should use process.cwd() for biome detection",
        );
        assert.ok(
          !formatSource.includes("findBiomeRoot"),
          "Should not have findBiomeRoot walk-up function",
        );
      },
    )
  )
  ...
  if (
    await asyncTest(
      "source contains biome check --write and BIOME_CONFIGS covers both extensions",
      async () => {
        const formatSource = fs.readFileSync(
          path.join(scriptsDir, "post-edit-format.js"),
          "utf8",
        ...
        assert.ok(
          formatSource.includes("@biomejs/biome"),
          "Should reference @biomejs/biome package",
        );
        assert.ok(
          formatSource.includes("check"),
          "Should include biome check subcommand",
        );
        assert.ok(
          formatSource.includes("--write"),
          "Should include --write flag",
        );
        assert.ok(
          formatSource.includes("biome.json"),
          "BIOME_CONFIGS should include biome.json",
        );
        assert.ok(
          formatSource.includes("biome.jsonc"),
  ...
  // ── Round 96: post-edit-format.js CLAUDE_PACKAGE_MANAGER support ──
  console.log(
    "\nRound 96: post-edit-format.js (CLAUDE_PACKAGE_MANAGER support):",
  );
  ...
  if (
    await asyncTest(
      "source contains RUNNERS map with npm, pnpm, yarn, bun",
      async () => {
        const formatSource = fs.readFileSync(
          path.join(scriptsDir, "post-edit-format.js"),
          "utf8",
        ...
        assert.ok(
          formatSource.includes("RUNNERS"),
          "Should define RUNNERS map",
        );
        assert.ok(
          formatSource.includes("CLAUDE_PACKAGE_MANAGER"),
          "Should read CLAUDE_PACKAGE_MANAGER env var",
        );
        for (const pm of ["npm", "pnpm", "yarn", "bun"]) {
          assert.ok(formatSource.includes(pm), `RUNNERS should include ${pm}`);
        }
      },
    )
  )
  ...
  if (
    await asyncTest(
      "exits 0 when CLAUDE_PACKAGE_MANAGER is set to pnpm",
      async () => {
        const testDir = createTestDir();
        ...
        assert.strictEqual(result.code, 0, "Should exit 0 with pnpm runner");
        cleanupTestDir(testDir);
      },
    )
  )
