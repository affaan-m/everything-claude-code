...
async function runTests() {
  ...
  if (
    test('observer-loop resumes wait when a trapped signal interrupts the claude child', () => {
      ...
      assert.ok(/wait_for_claude_analysis\s+"\$claude_pid"\s*\n\s*exit_code=\$\?/.test(observerLoopSource), 'observer-loop must call wait_for_claude_analysis to resume wait when SIGUSR1 interrupts an in-flight claude child');
      assert.ok(/wait_for_claude_analysis\(\)\s*\{[\s\S]*?while\s+true;\s*do[\s\S]*?wait\s+"\$child_pid"[\s\S]*?if\s+kill\s+-0\s+"\$child_pid"\s+2>\/dev\/null;\s*then\s*\n\s*continue/.test(observerLoopSource), 'wait_for_claude_analysis must run wait inside a loop and re-wait while the child is still alive so SIGUSR1 traps do not surface as false child failures');
    })
  )
    passed++;
  else failed++;
