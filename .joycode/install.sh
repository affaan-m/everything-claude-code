#!/bin/bash
#
# ECC JoyCode Installer
# Installs Everything Claude Code workflows into a JoyCode project.
#
# Usage:
#   ./install.sh              # Install to current directory
#   ./install.sh ~            # Install globally to ~/.joycode/
#
...
do_install() {
    ...
    # Create all required joycode subdirectories
    for dir in $SUBDIRS; do
        mkdir -p "$joycode_full_path/$dir"
    done
    ...
    if [ -d "$REPO_ROOT/commands" ]; then
        for f in "$REPO_ROOT/commands"/*.md; do
            ...
            target_path="$joycode_full_path/commands/$local_name"
    ...
    if [ -d "$REPO_ROOT/agents" ]; then
        for f in "$REPO_ROOT/agents"/*.md; do
            [ -f "$f" ] || continue
            local_name=$(basename "$f")
            target_path="$joycode_full_path/agents/$local_name"
    ...
    if [ -d "$REPO_ROOT/skills" ]; then
        for d in "$REPO_ROOT/skills"/*/; do
            ...
            target_skill_dir="$joycode_full_path/skills/$skill_name"
            skill_copied=0
    ...
    if [ -d "$REPO_ROOT/rules" ]; then
        while IFS= read -r rule_file; do
            ...
            target_path="$joycode_full_path/rules/$relative_path"
    ...
    for readme_file in "$SCRIPT_DIR/README.md" "$SCRIPT_DIR/README.zh-CN.md"; do
        if [ -f "$readme_file" ]; then
            ...
            target_path="$joycode_full_path/$local_name"
    ...
    echo "  Rules:     $rules"
    echo ""
    echo "Directory:   $(basename "$joycode_full_path")"
    echo ""
    echo "Next steps:"
    echo "  1. Open your project in JoyCode"
    echo "  2. Type / to see available commands"
    echo "  3. Enjoy the ECC workflows!"
    echo ""
    echo "To uninstall later:"
    echo "  cd $joycode_full_path"
    echo "  ./uninstall.sh"
}
