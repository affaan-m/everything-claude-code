 ...
 * and writes them to stderr as JSON-lines for the state store to capture.
 *
...
// Tools that represent security-relevant operations
 ...
 * Returns array of { name } for each detected pattern.
 */
...
function analyzeForGovernanceEvents(input, context = {}) {
  ...
  // 1. Secret detection in tool input and output content
  ...
  // 2. Approval-required commands (Bash tool only)
  if (toolName === 'Bash') {
  ...
  // 4. Security-relevant tool usage: elevated privilege commands
...
function run(rawInput) {
  ...
  try {
    ...
    if (events.length > 0) {
      // Write events to stderr as JSON-lines for the state store to capture.
