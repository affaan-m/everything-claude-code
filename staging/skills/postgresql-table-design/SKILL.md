...
description: Design a PostgreSQL-specific schema and optimize queries. Covers best-practices, data types, indexing, constraints, performance patterns, query optimization, monitoring, and advanced features
---
...
**Red flags in output**: `Seq Scan` on large tables (missing index), high `Rows Removed by Filter` (poor selectivity), `Sort Method: external merge` (increase `work_mem`), `Buffers: read >> hit` (cold cache).
...
SELECT ru.name, COALESCE(uo.order_count, 0) as orders
FROM recent_users ru
LEFT JOIN user_orders uo ON ru.id = uo.user_id;
```

Note: CTEs in PG12+ are automatically inlined when referenced once. Use `MATERIALIZED` to force materialization if the CTE result is reused or if the planner makes a bad choice.

### Cursor-Based Pagination

```sql
-- OFFSET is O(n) — scans all skipped rows. Use keyset/cursor pagination instead:
SELECT * FROM users
...
REFRESH MATERIALIZED VIEW CONCURRENTLY user_order_summary;  -- requires unique index
```
...
-- Approximate row count (fast alternative to COUNT(*))
